...
public class MixedListSwipeCallBack extends ItemTouchHelper.SimpleCallback {
    ...
    // 阻尼相关参数
    private static final float DAMP_THRESHOLD = 0.4f; //位置开始阻尼
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        Song song = extractSong(viewHolder);
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        // 重写此方法以控制滑动行为
        if (extractSong(viewHolder) != null) {
    ...
    private void handleRightSwipe(
        ...
        if (!isCurrentlyActive && !hasTriggered && progress >= 0.5f) {
            Song song = extractSong(viewHolder);
            if (song != null) {
                exoPlayManager.insertNext(song);
    ...
    @Nullable
    private Song extractSong(RecyclerView.ViewHolder holder) {
        if (holder instanceof SearchAdapter.SearchViewHolder) {
            return ((SearchAdapter.SearchViewHolder) holder).getSong();
        }
        return null;
    }
